...
# 修改配置
echo -e "[DolphinDB ODBC Driver]\nDescription=DolphinDB ODBC Driver\nDriver=/root/odbc_auto_test/workdir/libddbodbc.so" > /root/unixODBC/etc/odbcinst.ini
echo -e "[dolphindb]\nDriver=DolphinDB ODBC Driver\nServer=192.168.100.26\nDatabase=dfs://testdb\nPort=35998\nUid=admin\nPwd=123456" > /root/unixODBC/etc/odbc.ini
# 编译
...
# test
docker exec odbc_auto_test bash -c "cd /root/odbc_auto_test/workdir/codes/build && LD_LIBRARY_PATH=/root/odbc_auto_test/workdir:/root/unixODBC/lib ./BasicTest --gtest_output=xml:output.xml"
