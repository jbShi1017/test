...
mkdir -p $WORK_DIR/$PYTHON_VERSION/codes $WORK_DIR/$PYTHON_VERSION/workdir $WORK_DIR/$PYTHON_VERSION/single/web $WORK_DIR/$PYTHON_VERSION/cluster/web
# 准备server
if [ -d "$WORK_DIR/../prepare/$SERVER_VERSION" ]
then
    cp -Rf $WORK_DIR/../prepare/$SERVER_VERSION/* $WORK_DIR/$PYTHON_VERSION/single
    cp -Rf $WORK_DIR/../prepare/$SERVER_VERSION/* $WORK_DIR/$PYTHON_VERSION/cluster
else
    for i in libDolphinDB.so libgfortran.so.3 libstdc++.so.6 libtcmalloc_minimal.so.4
    do
        wget "ftp://ftp.dolphindb.cn/origin/release${SERVER_VERSION}/arm64/ALL/DolphinDBLib/$i" --ftp-user=ftpuser --ftp-password=DolphinDB123 -O $WORK_DIR/$PYTHON_VERSION/single/$i
        cp -f $WORK_DIR/$PYTHON_VERSION/single/$i $WORK_DIR/$PYTHON_VERSION/cluster/$i
    done
    wget "ftp://ftp.dolphindb.cn/origin/release${SERVER_VERSION}/arm64/ALL/DolphinDBExec/dolphindb" --ftp-user=ftpuser --ftp-password=DolphinDB123 -O $WORK_DIR/$PYTHON_VERSION/single/dolphindb
    cp -f $WORK_DIR/$PYTHON_VERSION/single/dolphindb $WORK_DIR/$PYTHON_VERSION/cluster/dolphindb
...
cp -Rf $WORK_DIR/../prepare/web/* $WORK_DIR/$PYTHON_VERSION/single/web
cp -Rf $WORK_DIR/../prepare/web/* $WORK_DIR/$PYTHON_VERSION/cluster/web
cp -Rf $WORK_DIR/../prepare/py_single_$PYTHON_VERSION/* $WORK_DIR/$PYTHON_VERSION/single
cp -Rf $WORK_DIR/../prepare/py_cluster_$PYTHON_VERSION/* $WORK_DIR/$PYTHON_VERSION/cluster
...
if [ -d "$WORK_DIR/../prepare/py_$PYTHON_BRANCH" ]
then
...
# 更新配置
cp -f $WORK_DIR/../prepare/py_settings_$PYTHON_VERSION.py $WORK_DIR/$PYTHON_VERSION/codes/test/setup/settings.py
